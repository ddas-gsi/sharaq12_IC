...
def minuitBraggFit(IC_dE_dX_ExptArr_iEvent):
    ...
    braggFit = ROOT.TF1("braggFit", "[1] + exp([2] * x) * (x < [0]) + [4] * exp(-((x - [0]) * (x - [0])) / (2 * [3] * [3])) * (x >= [0])", 0, n-1)
    ...
    # Perform Minuit Fit
    fit_result = graph.Fit(braggFit, "RQS")  # "RQS": Range, Quiet, Save
...
output_file = ROOT.TFile("/u/ddas/software/work/artemis-oedo/output/Analysis/physOutput/outputRange/1053_Analysis_2024_v2.root", "RECREATE")
...
for i_Event in range(num_entries):        # Change it later 
    ...
    # if FE9cut51Sc21.IsInside(PID_T_0, FE9_X_0) and S1cut51Sc21.IsInside(S1PID_T_0, S1_X_0) and S1_E_0 > 0.1:
        ...
        # if (np.sum(IC_E_ExptArr_iEvent[:8])/8.0) > 7:    # for 2022 data use this
        if (np.sum(IC_dE_dX_ExptArr_iEvent[:8])/8.0) > 6:  # for 2024 data use this  
            ...
            # best_params_iEvent = minuitBraggFit(IC_dE_dX_ExptArr_iEvent)  # minuit fit for single event
            best_params_iEvent = minuitBraggFit2(IC_dE_dX_ExptArr_iEvent)  # minuit fit for single event
            X0, Xpeak_best, Aexp_best, Bexp_best, sigmaGauss_best, ampGauss_best = best_params_iEvent
...
histFile = ROOT.TFile("/u/ddas/software/work/artemis-oedo/output/Analysis/physOutput/outputRange/1053_histFile_2024_v2.root", "RECREATE")
